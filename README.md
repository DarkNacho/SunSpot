...
**Important:** This Roboflow dataset is _not_ included in this Git repository and must be downloaded separately. A sample NOAA region file will be included as an example to demonstrate the image acquisition process within the
...
- **[datasets/data.yaml](datasets/data.yaml):** Configuration file for the YOLOv8 training process, specifying the location of training, validation, and testing images, as well as class names. _Note: This file is configured for the Roboflow dataset, which is not included in this repository._
- **[yolov8n.pt](yolov8n.pt):** Pre-trained YOLOv8 model.
- **sunspot_model/:** This directory contains the output from the YOLOv8 training run. Key files and subdirectories include:
...
```bash
pip install "sunpy[all]" "astropy[recommended]" opencv-python numpy pandas matplotlib ultralytics
